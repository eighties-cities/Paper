...
### Maps of the synthetic population by demographic category in the Paris Region (night cells) (Figure 1)
...
Median values (and Q1;Q3) of socialInequality in healthy diet for the 5 scenarios (table 5)
```{r}
socialIneq_distrib_perScenario <- sim %>% 
  group_by(numOfScenario) %>%
  
  summarise(median = round(quantile(socialInequality, probs = 0.5),3),
            Q1 = round(quantile(socialInequality, probs = 0.25),3),
            Q3 = round(quantile(socialInequality, probs = 0.75),3))

socialIneq_distrib_perScenario
```

Median values(and Q1;Q3)  of proportions of agents with healthy behaviour for the 5 scenarios (table 5)
```{r}
totAgents <- sum(pop_0$pop)
totAgents

propHealthy_distrib_perScenario <- sim %>% 
  group_by(numOfScenario) %>%
  
  summarise(median = round(quantile(numberOfHealthy/totAgents*100, probs = 0.5),2),
            Q1 = round(quantile(numberOfHealthy/totAgents*100, probs = 0.25),2),
            Q3 = round(quantile(numberOfHealthy/totAgents*100, probs = 0.75),2))

propHealthy_distrib_perScenario
```
...
## Evolution over the six series of three time slices in simulated values - 5aDay model (Appendix 8)
...
## Alternative measures of spatial distribution of agents with healthy behaviour - in the day and evening cells (not showns) 
...
Combine Duncan and Moran files for Day cells 
...
Combine Duncan and Moran files for in Evening cells 
